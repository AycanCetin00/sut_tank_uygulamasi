  ...
  Future<void> cihazlariGetir() async {
    try {
      final response = await http.get(
        Uri.parse('http://10.56.8.249:5266/api/CihazKullanici/${widget.kullaniciId}'),
      );
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          cihazListesi = cihazlar.map((cihaz) => {
            'cihazId': cihaz['cihazId'],  // Cihaz ID
            'cihazAdi': cihazAdiOlustur(cihaz['cihazId']),  // Cihaz adını ID'ye göre oluştur
          }).toList();
        });
      } else {
  ...
  void cihazEkle(String cihazAdi) {
    setState(() {
      if (cihazAdi.isNotEmpty && !cihazListesi.any((cihaz) => cihaz['cihazAdi'] == cihazAdi)) {
        cihazListesi.add({'cihazAdi': cihazAdi}); // Cihaz adını listeye ekle
      }
    });
  }
                ...
                child: ElevatedButton(
                  onPressed: () {
                    ...
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => CihazDetaySayfasi(cihazId: cihaz['cihazId'].toString()), // Cihaz ID'yi gönder
                      ),
                    );
                  },
                  style: ElevatedButton.styleFrom(
                    ...
                    backgroundColor: Colors.teal.shade700,
                    minimumSize: Size(MediaQuery.of(context).size.width * 0.95, 60),
                    maximumSize: Size(MediaQuery.of(context).size.width * 0.95, 60),
                  ),
                  child: Text(
                    cihaz['cihazAdi'],
                    style: TextStyle(
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          cihazListesi = cihazlar.map((cihaz) => {
            'cihazId': cihaz['cihazId'],  // Cihaz ID
            'cihazAdi': cihazAdiOlustur(cihaz['cihazId']),  // Cihaz adını ID'ye göre oluştur
            'aktifMi': false,  // Başlangıçta cihaz aktif değil
          }).toList();
        });
      } else {
        print('Cihazlar getirilemedi: ${response.statusCode}');
  ...
  Future<void> cihazDurumunuGuncelle(int cihazId, bool yeniDurum) async {
    try {
      ...
      final response = await http.put(
        Uri.parse('http://10.56.8.249:5266/api/CihazGet/$cihazId'), // Gerekirse endpointi değiştirin
